...
public class LoadBalancer {
	...
	public LoadBalancer(ServerSocket socket, DataCenterInstance[] instances) throws IOException {
		...
		for (int i = 0; i < reservations.size(); i++) {
			...
			if(ist.get(0).getState().getName().equals("running")) {
				...
				System.out.println("subnetID is: " + subnetID + ", sg ID is: " + sgID);
				break;
			}
		}
		
		//hold current healthy running DCIs
	...
	public void start() throws IOException {
		...
		while (true) {
			int i = 0;
			//send requests to all of them 5 times
			for(int j = 0; j < 5; j++) {
				for(i = 0; i < runningList.size(); i++) {
					Runnable requestHandler = new RequestHandler(socket.accept(), runningList.get(i));
	...
	/* add data center instances to load gen testing */
